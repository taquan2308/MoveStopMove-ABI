...
public class GameManager2 : MonoBehaviour
{
    ...
    public void SpawEnemy()
    {
        ...
        if (indexPrefabsInArrSpawn < arrEnemyPrefabs.Length)
        {
            ...
            {
                if (enemy[i].transform.position.z < pointCenterMapTrans.position.z && enemy[i].transform.position.x < pointCenterMapTrans.position.x)
                {
                    topLeft += 1;
                }
                else if (enemy[i].transform.position.z > pointCenterMapTrans.position.z && enemy[i].transform.position.x < pointCenterMapTrans.position.x)
                {
                    topRight += 1;
                }
                else if (enemy[i].transform.position.z > pointCenterMapTrans.position.z && enemy[i].transform.position.x > pointCenterMapTrans.position.x)
